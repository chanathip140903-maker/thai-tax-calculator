    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="bg-primary text-primary-foreground py-10 px-6 mb-8">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-3xl font-bold mb-2">คำนวณภาษี ภ.ง.ด. 90</h1>
          <p className="text-primary-foreground/80 text-lg">สำหรับผู้มีรายได้หลากหลายประเภท (มาตรา 40(1) ถึง 40(8))</p>
        </div>
      </div>
          ...
          <div className="bg-card p-8 rounded-2xl shadow-sm border border-border space-y-10">
            <div className="border-b border-border pb-6">
              ...
              <div className="flex flex-wrap gap-3 mt-6">
                <button onClick={() => toggleSection('t1_2')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t1_2 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
                  40(1),(2) เงินเดือน/รับจ้าง
                </button>
                <button onClick={() => toggleSection('t3')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t3 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
                  40(3) ลิขสิทธิ์
                </button>
                <button onClick={() => toggleSection('t4')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t4 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
                  40(4) ดอกเบี้ย/ปันผล
                </button>
                <button onClick={() => toggleSection('t5')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t5 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
                  40(5) ค่าเช่า
                </button>
                <button onClick={() => toggleSection('t6')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t6 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
                  40(6) วิชาชีพอิสระ
                </button>
                <button onClick={() => toggleSection('t7_8')} className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showIncome.t7_8 ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-muted border-border'}`}>
            ...
            {showIncome.t3 && (
              <FormSection title="ค่าลิขสิทธิ์ (มาตรา 40(3))" description="หักค่าใช้จ่ายได้ 50% ไม่เกิน 100,000 บาท">
                <Controller control={form.control} name="income.type40_3" render={({ field }) => (
                  <NumberInput label="ค่าลิขสิทธิ์ กู๊ดวิลล์" value={field.value} onChangeValue={field.onChange} />
                )}/>
              </FormSection>
            )}
